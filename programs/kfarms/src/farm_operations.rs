...
pub fn refresh_global_reward(
    ...
    let amount: u64 = {
        ...
        let decimal_adjusted_amt = u128::from(
            u128_mul_div(
                reward_type_amt,
                1,
                u128::from(ten_pow(rps_decimals)),
            )
            .map_err(|_| dbg_msg!(FarmError::IntegerOverflow))?,
        );
        ...
        let oracle_adjusted_amt = if farm_state.scope_oracle_price_id == u64::MAX {
            ...
            if ts - price.unix_timestamp > farm_state.scope_oracle_max_age {
                ...
                let factor = ten_pow(exp_usize) as u128;
...

    {
        ...
        // Debug-only invariant: issued rewards match per-share change within rounding
        #[cfg(debug_assertions)]
        {
            use decimal_wad::decimal::Decimal as D;
            let denom = if farm_state.is_delegated() {
                D::from_scaled_val(farm_state.total_active_stake_scaled)
            } else {
                farm_state.get_total_active_stake_decimal()
            };
            let approx_issued: u64 = (added_reward_per_share * denom)
                .try_floor()
                .unwrap_or(0);
            debug_assert!(
                approx_issued <= rewards,
                "approx_issued={} rewards={}",
                approx_issued,
                rewards
            );
